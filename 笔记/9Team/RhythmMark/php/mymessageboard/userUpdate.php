...
if(!empty($_POST))
{
	$connect  = mysqli_connect( 'localhost', 'root', 'admin', 'renwusi');
	$avatar = '';
	...
	if(!empty($_FILES['avatar']) && $_FILES['avatar']['error'] == 0 )
	{
		...
		if($move === true)
		{
			$avatar = "http://localhost/githublyb/images/{$avatar['name']}";
		}
	}
	$sqlupdate ="  UPDATE `renwusi`.`user` SET `name`='{$name}', `avatar`='{$avatar}' WHERE `id`='{$userInfo['id']}'; ";
